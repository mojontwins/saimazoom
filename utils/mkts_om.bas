' mkts_om v0.9.20230331 - Converts graphics
' Copyleft 2017-2023 The Mojon Twins
...
Sub doPals (img As Any Ptr, prefix As String, outputFn As String)
	...
	Select Case platform
		Case PLATFORM_CPC:
			...
			Print #f, "// Generated by mkts_om v0.9.20230331"
			Print #f, "// Copyleft 2017-2023 by The Mojon Twins"
			Print #f, ""
...
Sub doChars (img As Any Ptr, xc0 As Integer, yc0 As Integer, w As Integer, h As Integer, max As Integer)
	...
	Puts "mkts_om v0.9.20230331 ~ Chars mode, " & ct & " patterns extracted (" & (ct * patternSize) & " bytes)."
End Sub
...
Sub doStrait2x2(img As Any Ptr, xc0 As Integer, yc0 As Integer, w As Integer, h As Integer, max As Integer)
	...
	Puts "mkts_om v0.9.20230331 ~ Strait2x2 mode, " & ct*4 & " patterns extracted (" & (ct*4 * patternSize) & " bytes)."
End Sub
...
Sub cpcDoSuperBuffer (img As Any Ptr)
	...
	Puts "mkts_om v0.9.20230331 ~ Superbuffer mode (12288 bytes)."
End Sub
...
Sub cpcDoScr (img As Any Ptr)
	...
	Puts "mkts_om v0.9.20230331 ~ Superbuffer mode (16384 bytes)."
...
Sub doTmaps (img As Any Ptr, xc0 As Integer, yc0 As Integer, w As Integer, h As Integer, wMeta As Integer, hMeta As Integer, max As Integer)
	...
	Puts "mkts_om v0.9.20230331 ~ Tmaps mode, " & ct & " patterns extracted (" & (ct * patternSize) & " bytes) from " & ctTmaps & " metatiles (" & (ctTmaps * i * wMeta * hMeta) & " bytes)."
End Sub
...
zxDoSpritesBreak:
	...
	cutSpriteCount = ct
	Puts "mkts_om v0.9.20230331 ~ Sprites mode, " & ct & " " & wMetaPixels & "x" & hMetaPixels & " sprite cells with masks extracted (" & (mainIndex) & " bytes)."
End Sub
...
cpcDoSpritesBreak:
	cutSpriteCount = ct
	Puts "mkts_om v0.9.20230331 ~ Sprites mode, " & ct & " " & wMetaPixels & "x" & hMetaPixels & " sprite cells extracted (" & (mainIndex - curIndex) & " bytes)."
End Sub
...
Sub generateStraitMappings (mappingsFn As String, wMeta As Integer, hMeta As Integer, max As Integer)
	...
	If wMeta = 3 And hMeta = 3 Then
		cox = 0: coy = 0
		functionSuffix = "12x24"
		ca = -1
		gng = -1
	Else
...
Sub zxDoBg (img As Any Ptr, xc0 As Integer, yc0 As Integer, w As Integer, h As Integer)
	...
	Puts "mkts_om v0.9.20230331 ~ BG mode, " & ct & " patterns (" & (8*ct) & " bytes). NT is " & auxIndex & " bytes."
End Sub
...
Sub cpcDoNametable (img As Any Ptr)
	...
	Puts "mkts_om v0.9.20230331 ~ Nametable mode, " & ct & " patterns extracted (" & (ct * patternSize) & " bytes)."
End Sub
...
Sub doNametableRLE (img As Any Ptr)
	...
	Puts "mkts_om v0.9.20230331 ~ Rle encoder, compressed to " & rleIdx & " bytes."
End Sub
...
Select Case Lcase (sclpGetValue ("mode"))
	...
	Case "mapped"
		doTmaps img, xc0, yc0, w, h, wMeta, hMeta, max
		writeFullBinary outputBaseFn & ".bin" 
		if mappingsFn <> "" Then writeTsmaps mappingsFn
		ImageDestroy img
